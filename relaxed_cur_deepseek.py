...
class RelaxedCURCompressor:
    ...
    def _compress_expert_worker(
        self,
        args: Tuple[int, nn.Module, torch.device, torch.dtype],
    ) -> Tuple[int, RelaxedCURExpert, Dict]:
        exp_idx, expert, target_device, target_dtype = args
        
        compressed_expert, stats = self.compress_expert(expert, verbose=False)
        compressed_expert = compressed_expert.to(device=target_device, dtype=target_dtype)
        
        return exp_idx, compressed_expert, stats
    ...
    def compress_moe_layer(
        ...
        if parallel and n_experts > 1:
            work_items = [
                (exp_idx, moe.experts[exp_idx], target_device, original_dtype)
                for exp_idx in range(n_experts)
            ]
            
            effective_workers = min(self.num_workers, n_experts, 2)
            
            with ThreadPoolExecutor(max_workers=effective_workers) as executor:
                futures = {
                    executor.submit(self._compress_expert_worker, item): item[0]
                    for item in work_items
                }
                
                with tqdm(total=n_experts, desc="    Compressing experts", leave=False) as pbar:
                    for future in as_completed(futures):
                        exp_idx, compressed_expert, stats = future.result()
...
class InterpretabilityAnalyzer:
    ...
    @staticmethod
    def analyze_cosine_similarity(compressed_moe: RelaxedCURMoE) -> Dict:
        all_col_sims = []
        all_row_sims = []
    ...
    @staticmethod
    def analyze_offset_distribution(compressed_moe: RelaxedCURMoE) -> Dict:
        all_col_offsets = []
        all_row_offsets = []
...
def feature_ablation_study(
    ...
    n_features_to_ablate: int = 50,
) -> Dict:
    print("\nFEATURE ABLATION STUDY")
    print("""
    We zero out top CUR-selected features and measure PPL impact.
    If CUR features hurt more than random → selection is meaningful.
    """)
    
    top_features = top_features[:n_features_to_ablate]
    n_features = len(top_features)
    
    print(f"  Ablating {n_features} features across {len(selected_layers)} layers")
    ...
    for layer_idx in selected_layers:
        ...
        for exp_idx, expert in enumerate(layer.mlp.experts):
            original_weights[layer_idx][exp_idx] = {
                'gate': expert.gate_proj.weight.data.clone(),
                'up': expert.up_proj.weight.data.clone(),
            }
            if hidden_size is None:
                hidden_size = expert.gate_proj.weight.shape[1]
    
    if not original_weights:
        print("  No MoE layers found, skipping ablation study")
        return {}
    
    if hidden_size is None:
        print("  Could not determine hidden size, skipping ablation study")
        return {}
    
    print(f"\n  Ablating top {n_features} CUR-selected features across {len(original_weights)} layers...")
    
    for layer_idx in original_weights.keys():
        layer = model.model.layers[layer_idx]
        for exp_idx, expert in enumerate(layer.mlp.experts):
            for feat_idx in top_features:
                if feat_idx < hidden_size:
                    expert.gate_proj.weight.data[:, feat_idx] = 0
                    expert.up_proj.weight.data[:, feat_idx] = 0
    
    top_ablated_ppl = evaluate_perplexity(model, tokenizer, max_samples=100)
    top_ppl_increase = (top_ablated_ppl - baseline_ppl) / baseline_ppl * 100
    print(f"  PPL after ablating TOP features: {top_ablated_ppl:.4f} (+{top_ppl_increase:.2f}%)")
    
    for layer_idx in original_weights.keys():
        layer = model.model.layers[layer_idx]
        for exp_idx, expert in enumerate(layer.mlp.experts):
            expert.gate_proj.weight.data = original_weights[layer_idx][exp_idx]['gate'].clone()
            expert.up_proj.weight.data = original_weights[layer_idx][exp_idx]['up'].clone()
    
    random_ppl_increases = []
    
    for trial in range(n_random_trials):
        torch.manual_seed(42 + trial)
        random_features = torch.randperm(hidden_size)[:n_features].tolist()
        ...
        random_ppl = evaluate_perplexity(model, tokenizer, max_samples=100)
        random_increase = (random_ppl - baseline_ppl) / baseline_ppl * 100
        random_ppl_increases.append(random_increase)
        print(f"    Trial {trial+1}: Random features PPL = {random_ppl:.4f} (+{random_increase:.2f}%)")
    ...
    avg_random_increase = np.mean(random_ppl_increases)
    std_random_increase = np.std(random_ppl_increases)
    print(f"\n  PPL after ablating RANDOM features: +{avg_random_increase:.2f}% ± {std_random_increase:.2f}% (avg of {n_random_trials} trials)")
    
    if avg_random_increase > 0.1:
        ratio = top_ppl_increase / avg_random_increase
    elif top_ppl_increase > 0.1:
        ratio = float('inf')
    else:
        ratio = 1.0
    
    print(f"\n  RESULT:")
    print(f"    Top CUR features: +{top_ppl_increase:.2f}% PPL")
    print(f"    Random features:  +{avg_random_increase:.2f}% PPL")
    
    if ratio != float('inf'):
        print(f"    Ratio: {ratio:.2f}x")
    else:
        print(f"    Ratio: inf (random had no impact)")
    
    if top_ppl_increase > avg_random_increase * 1.5 and top_ppl_increase > 0.5:
        print(f"    ✅ CUR features are {ratio:.1f}x MORE important than random!")
        verdict = "strong"
    elif top_ppl_increase > avg_random_increase * 1.1 and top_ppl_increase > 0.1:
        print(f"    ⚠️ CUR features are slightly more important ({ratio:.1f}x)")
        verdict = "weak"
    elif top_ppl_increase < 0.1 and avg_random_increase < 0.1:
        print(f"    ⚠️ Both have minimal impact - features may not be critical in these layers")
        verdict = "inconclusive"
    else:
        print(f"    ❌ CUR features are NOT more important than random")
        verdict = "negative"
    
    return {
        'baseline_ppl': baseline_ppl,
        'top_ablated_ppl': top_ablated_ppl,
        'top_ppl_increase_pct': top_ppl_increase,
        'random_ppl_increases': random_ppl_increases,
        'avg_random_ppl_increase_pct': avg_random_increase,
        'std_random_ppl_increase_pct': std_random_increase,
        'importance_ratio': ratio if ratio != float('inf') else -1,
        'top_features': top_features,
        'n_features_ablated': n_features,
        'n_layers_ablated': len(original_weights),
        'verdict': verdict,
    }


def analyze_expert_clustering(
    compressed_moe: RelaxedCURMoE,
    top_k: int = 100,
) -> Dict:
    ...
    expert_feature_sets = {}
    for exp_idx, expert in enumerate(compressed_moe.experts):
        features = set(expert.gate_proj.col_indices[:top_k])
        expert_feature_sets[exp_idx] = features
    
    similarity_matrix = np.zeros((n_experts, n_experts))
    for i in range(n_experts):
        for j in range(n_experts):
            intersection = len(expert_feature_sets[i] & expert_feature_sets[j])
            union = len(expert_feature_sets[i] | expert_feature_sets[j])
    ...
    print(f"\n  Most SIMILAR expert pairs (share features):")
    for i, j, sim in similarities[:5]:
        shared = expert_feature_sets[i] & expert_feature_sets[j]
        print(f"    Experts {i} & {j}: {sim:.3f} Jaccard ({len(shared)} shared features)")
    
    print(f"\n  Most DIFFERENT expert pairs (distinct roles):")
    for i, j, sim in similarities[-5:]:
        print(f"    Experts {i} & {j}: {sim:.3f} Jaccard")
    
    upper_triangle = [similarities[k][2] for k in range(len(similarities))]
    avg_similarity = np.mean(upper_triangle)
    
    print(f"\n  Average expert similarity: {avg_similarity:.3f}")
    print(f"  Interpretation: {'Experts share many features' if avg_similarity > 0.3 else 'Experts are specialized (diverse)'}")
    ...
    for i in range(n_experts):
        ...
        assigned.add(i)
        for j in range(i + 1, n_experts):
            if j not in assigned and similarity_matrix[i, j] > 0.4:
                cluster.append(j)
                assigned.add(j)
        if len(cluster) > 1:
            clusters.append(cluster)
    
    print(f"\n  Found {len(clusters)} expert clusters (similarity > 0.4):")
    for idx, cluster in enumerate(clusters[:5]):
        print(f"    Cluster {idx}: Experts {cluster}")
    
    return {
        'similarity_matrix': similarity_matrix.tolist(),
        'avg_similarity': avg_similarity,
        'most_similar_pairs': [(i, j, float(s)) for i, j, s in similarities[:10]],
        'most_different_pairs': [(i, j, float(s)) for i, j, s in similarities[-10:]],
        'clusters': clusters,
...
def rank_sensitivity_analysis(
    W: torch.Tensor,
    ranks: List[int] = [256, 384, 512, 640, 768],
    ...
    print(f"\n  Testing ranks on weight matrix shape: {tuple(W.shape)}")
    
    for rank in ranks:
        if rank >= min(W.shape):
            print(f"\n  Rank {rank}: Skipping (>= matrix dimension)")
            continue
            
        print(f"\n  Rank {rank}:")
        
        result = relaxed_cur_decompose(
            W, rank=rank, n_iterations=n_iterations,
            lr=lr, lambda_reg=lambda_reg, verbose=False,
            device=device
        )
        ...
        avg_cosine = np.mean(result.col_cosine_sims)
        avg_offset = np.mean(result.col_offset_norms)
        
        print(f"    Error: {result.final_error:.4f}")
        print(f"    Compression: {compression*100:.1f}%")
        print(f"    Cosine sim: {avg_cosine:.4f}")
        print(f"    Offset norm: {avg_offset:.4f}")
        ...
        results.append({
            ...
            'avg_offset_norm': avg_offset,
        })
    
    print(f"\n  SUMMARY TABLE:")
    print(f"  {'Rank':<8} {'Error':<10} {'Compress':<12} {'Cosine':<10} {'Offset':<10}")
    for r in results:
        print(f"  {r['rank']:<8} {r['error']:<10.4f} {r['compression']*100:<12.1f}% "
              f"{r['avg_cosine_sim']:<10.4f} {r['avg_offset_norm']:<10.4f}")
    
    return {'rank_analysis': results}


def layerwise_interpretability_analysis(
    model: nn.Module,
    selected_layers: List[int],
) -> Dict:
    print("\nLAYER-WISE INTERPRETABILITY TREND")
    ...
    for layer_idx in selected_layers:
        ...
        for expert in compressed_moe.experts:
            for proj_name in ['gate_proj', 'up_proj', 'down_proj']:
                proj = getattr(expert, proj_name)
                all_cosines.extend(proj.col_cosine_sims)
                all_offsets.extend(proj.col_offset_norms)
            all_features.extend(expert.gate_proj.col_indices[:30])
        ...
        metrics = {
            ...
            'avg_offset': np.mean(all_offsets),
            'unique_features': unique_features,
            'pct_above_0.8': sum(1 for c in all_cosines if c > 0.8) / len(all_cosines),
        }
        layer_metrics.append(metrics)
        
        print(f"  Layer {layer_idx}: cosine={metrics['avg_cosine']:.4f} (±{metrics['std_cosine']:.4f}), "
              f"offset={metrics['avg_offset']:.4f}, unique_features={unique_features}")
    
    if len(layer_metrics) > 1:
        cosines = [m['avg_cosine'] for m in layer_metrics]
        offsets = [m['avg_offset'] for m in layer_metrics]
        
        cosine_trend = "decreasing" if cosines[-1] < cosines[0] - 0.01 else (
            "increasing" if cosines[-1] > cosines[0] + 0.01 else "stable"
        )
        
        print(f"\n  TREND ANALYSIS:")
        print(f"    Cosine similarity trend: {cosine_trend}")
        print(f"    First layer: {cosines[0]:.4f}, Last layer: {cosines[-1]:.4f}")
        print(f"    Offset range: {min(offsets):.4f} - {max(offsets):.4f}")
    
    return {'layer_metrics': layer_metrics}


@torch.no_grad()
def feature_activation_analysis(
    model: nn.Module,
    tokenizer,
    top_features: List[int],
    layer_idx: int = 1,
    ...
    try:
        ...
        texts = [t for t in data['text'] if len(t) > 100][:n_samples]
    except Exception as e:
        print(f"  Could not load WikiText dataset: {e}")
        return {}
    
    if len(texts) == 0:
        print("  No valid texts found, skipping activation analysis")
        return {}
    ...
    top_feature_set = set(top_features)
    other_features = [i for i in range(hidden_size) if i not in top_feature_set][:len(top_features)]
    
    if len(other_features) == 0:
        print("  Not enough other features to compare, skipping")
        return {}
    ...
    if len(activations) == 0:
        print("  No activations captured, skipping")
        return {}
    
    for act in activations:
        act = act.view(-1, hidden_size).float()
        
        top_feat_valid = [f for f in top_features if f < act.shape[1]]
        other_feat_valid = [f for f in other_features if f < act.shape[1]]
        
        if len(top_feat_valid) == 0 or len(other_feat_valid) == 0:
            continue
        
        top_act = act[:, top_feat_valid].abs().mean().item()
        other_act = act[:, other_feat_valid].abs().mean().item()
        
        top_activations.append(top_act)
        other_activations.append(other_act)
    
    if len(top_activations) == 0:
        print("  No valid activations to analyze")
        return {}
    
    avg_top = np.mean(top_activations)
    avg_other = np.mean(other_activations)
    ratio = avg_top / avg_other if avg_other > 0 else float('inf')
    
    print(f"\n  Top CUR features ({len(top_features)}) avg activation: {avg_top:.6f}")
    print(f"  Other features ({len(other_features)}) avg activation:   {avg_other:.6f}")
    print(f"  Ratio: {ratio:.2f}x")
    
    if ratio > 1.2:
        print(f"  ✅ CUR-selected features have {ratio:.1f}x higher activations!")
        verdict = "strong"
    elif ratio > 1.05:
        print(f"  ⚠️ CUR features have slightly higher activations ({ratio:.2f}x)")
        verdict = "weak"
    else:
        print(f"  ❌ CUR features do NOT have notably higher activations")
        verdict = "negative"
    
    return {
        'avg_top_activation': avg_top,
        'avg_other_activation': avg_other,
...
def run_comprehensive_analysis(
    ...
    print("\n1. COSINE SIMILARITY (Direction Preservation)")
    print("""
    High cosine similarity means column DIRECTION is preserved.
    - 1.0 = exactly same direction (fully interpretable)
    - 0.8+ = mostly same direction (interpretable)
    - 0.5-0.8 = somewhat preserved
    - <0.5 = significantly changed
    """)
    
    all_cosine_results = {'all_col_sims': [], 'all_row_sims': []}
    ...
    for layer_idx in selected_layers:
        ...
        print(f"\n  Layer {layer_idx}:")
        print(f"    Column cosine similarity:")
        stats = cosine_results['column_cosine_similarity']
        print(f"      Min:    {stats['min']:.4f}")
        print(f"      Max:    {stats['max']:.4f}")
        print(f"      Mean:   {stats['mean']:.4f}")
        print(f"      Median: {stats['median']:.4f}")
        print(f"      >0.9:   {stats['pct_above_0.9']*100:.1f}%")
        print(f"      >0.8:   {stats['pct_above_0.8']*100:.1f}%")
        print(f"      >0.7:   {stats['pct_above_0.7']*100:.1f}%")
        
        analysis_results['layers'][layer_idx] = {
        ...
        for expert in compressed_moe.experts:
            for proj_name in ['gate_proj', 'up_proj', 'down_proj']:
                proj = getattr(expert, proj_name)
                all_cosine_results['all_col_sims'].extend(proj.col_cosine_sims)
                all_cosine_results['all_row_sims'].extend(proj.row_cosine_sims)
    
    all_col_sims = all_cosine_results['all_col_sims']
    if len(all_col_sims) > 0:
        analysis_results['global']['cosine_similarity'] = {
            'mean': np.mean(all_col_sims),
            'median': np.median(all_col_sims),
            'std': np.std(all_col_sims),
            'min': min(all_col_sims),
    ...
    for layer_idx in selected_layers:
        ...
        print(f"    Column offset norms (relative to original):")
        print(f"      Min:    {stats['min']:.4f} ({stats['min']*100:.1f}%)")
        print(f"      Max:    {stats['max']:.4f} ({stats['max']*100:.1f}%)")
        print(f"      Mean:   {stats['mean']:.4f} ({stats['mean']*100:.1f}%)")
        print(f"      Median: {stats['median']:.4f} ({stats['median']*100:.1f}%)")
        
        print(f"    Effectively exact columns:")
        for thresh, data in offset_results['effectively_exact_columns'].items():
            print(f"      {thresh}: {data['count']:,} / {data['total']:,} ({data['ratio']*100:.1f}%)")
        
        analysis_results['layers'][layer_idx]['offset_distribution'] = offset_results
    ...
    if len(all_offsets) > 0:
        analysis_results['global']['offset_distribution'] = {
            'mean': np.mean(all_offsets),
            'median': np.median(all_offsets),
            'std': np.std(all_offsets),
            'min': min(all_offsets),
            'max': max(all_offsets),
            'effectively_exact': {
                f'<{int(t*100)}%': sum(1 for o in all_offsets if o < t) / len(all_offsets)
    ...
    print("""
    This analysis is IMPOSSIBLE with SVD.
    We can identify which features each expert prioritizes.
    """)
    ...
    for layer_idx in selected_layers:
        ...
        print(f"\n  Layer {layer_idx}:")
        print(f"    Total unique features selected: {spec_results['total_unique_features']}")
        print(f"    Experts with unique features: {spec_results['experts_with_unique']}/{spec_results['n_experts']}")
        
        print(f"\n    Top shared features (selected by many experts):")
        for feat, count in spec_results['shared_features'][:5]:
            print(f"      Feature {feat}: {count}/{spec_results['n_experts']} experts")
            all_shared_features.append(feat)
        
        print(f"\n    Example unique features:")
        shown = 0
        for exp_idx, features in spec_results['unique_features'].items():
            if features and shown < 3:
                print(f"      Expert {exp_idx}: {features[:5]}")
                shown += 1
        
        analysis_results['layers'][layer_idx]['expert_specialization'] = spec_results
    
    if len(selected_layers) > 1:
        print("\n4. CROSS-LAYER CONSISTENCY")
        print("""
        Checking if same features are important across layers.
        High consistency = fundamental feature importance.
        """)
        
        cross_layer = InterpretabilityAnalyzer.analyze_cross_layer_consistency(
            model, selected_layers, top_k=30
        )
        
        print(f"\n  Features appearing in multiple layers:")
        print(f"    Total unique features: {cross_layer['total_unique_features']}")
        print(f"    Features in all layers: {cross_layer['features_in_all_layers']}")
        
        print(f"\n    Top cross-layer features:")
        for item in cross_layer['cross_layer_features'][:10]:
            print(f"      Feature {item['feature']}: in {item['layers_present']}/{cross_layer['n_layers']} layers "
                  f"(total count: {item['total_count']})")
        
        analysis_results['global']['cross_layer_consistency'] = cross_layer
    
    if run_ablations and tokenizer is not None:
        all_top_features = list(dict.fromkeys(all_shared_features))
        
        for layer_idx in selected_layers:
            if layer_idx in analysis_results['layers']:
                layer_spec = analysis_results['layers'][layer_idx].get('expert_specialization', {})
                for feat, count in layer_spec.get('shared_features', []):
                    if feat not in all_top_features:
                        all_top_features.append(feat)
        
        print(f"\n  Collected {len(all_top_features)} unique top features for ablation")
        
        if len(all_top_features) > 0:
            ablation_results = feature_ablation_study(
                original_model, tokenizer, 
                top_features=all_top_features[:100],
                selected_layers=selected_layers,
                n_random_trials=3,
                n_features_to_ablate=50,
            )
            analysis_results['ablations']['feature_ablation'] = ablation_results
        
        first_layer = model.model.layers[selected_layers[0]]
        if hasattr(first_layer.mlp, 'experts'):
            clustering_results = analyze_expert_clustering(first_layer.mlp)
            analysis_results['ablations']['expert_clustering'] = clustering_results
        
        layerwise_results = layerwise_interpretability_analysis(model, selected_layers)
        analysis_results['ablations']['layerwise_trend'] = layerwise_results
        
        if len(all_top_features) > 0:
            activation_results = feature_activation_analysis(
                original_model, tokenizer, all_top_features[:50],
                layer_idx=selected_layers[0], n_samples=10
            )
            analysis_results['ablations']['feature_activation'] = activation_results
        
        first_layer_orig = original_model.model.layers[selected_layers[0]]
        if hasattr(first_layer_orig.mlp, 'experts'):
            W_sample = first_layer_orig.mlp.experts[0].gate_proj.weight.data
            rank_results = rank_sensitivity_analysis(
                W_sample, ranks=[256, 384, 512, 640],
                n_iterations=n_iterations, lr=lr, lambda_reg=lambda_reg,
                device=device
            )
            analysis_results['ablations']['rank_sensitivity'] = rank_results
    
    print("\nPAPER-READY SUMMARY")
    
    global_cosine = analysis_results['global'].get('cosine_similarity', {})
    global_offset = analysis_results['global'].get('offset_distribution', {})
    
    if global_cosine and global_offset:
        print(f"""
KEY FINDINGS (all verifiable from above data):

1. DIRECTION PRESERVATION (Cosine Similarity):
   - Mean cosine similarity: {global_cosine.get('mean', 0):.4f}
   - {global_cosine.get('pct_above_0.7', 0)*100:.1f}% of columns have >0.7 cosine similarity
   - {global_cosine.get('pct_above_0.8', 0)*100:.1f}% of columns have >0.8 cosine similarity
   
   INTERPRETATION: 
   {"Columns mostly preserve direction - interpretability is meaningful" if global_cosine.get('mean', 0) > 0.7 else "Columns undergo significant direction change"}

2. OFFSET MAGNITUDE:
   - Mean offset: {global_offset.get('mean', 0)*100:.1f}% of original column norm
   - {global_offset.get('effectively_exact', {}).get('<10%', 0)*100:.1f}% columns have <10% adjustment
   - {global_offset.get('effectively_exact', {}).get('<30%', 0)*100:.1f}% columns have <30% adjustment
""")
    
    if 'feature_ablation' in analysis_results.get('ablations', {}):
        fa = analysis_results['ablations']['feature_ablation']
        print(f"""
3. FEATURE ABLATION ({fa.get('n_features_ablated', 0)} features across {fa.get('n_layers_ablated', 0)} layers):
   - Top features ablation: +{fa.get('top_ppl_increase_pct', 0):.2f}% PPL
   - Random features ablation: +{fa.get('avg_random_ppl_increase_pct', 0):.2f}% PPL
   - Importance ratio: {fa.get('importance_ratio', 0):.2f}x
   - Verdict: {fa.get('verdict', 'unknown').upper()}
""")
    
    if 'feature_activation' in analysis_results.get('ablations', {}):
        act = analysis_results['ablations']['feature_activation']
        if act:
            print(f"""
4. FEATURE ACTIVATION:
   - Top features avg activation: {act.get('avg_top_activation', 0):.6f}
   - Other features avg activation: {act.get('avg_other_activation', 0):.6f}
   - Activation ratio: {act.get('activation_ratio', 0):.2f}x
   - Verdict: {act.get('verdict', 'unknown').upper()}
""")
    
    print("""
5. COMPARISON TO SVD:
   - SVD: Abstract vectors, 0% interpretability
   - Relaxed CUR: Real features, direction preserved
   
   We provide insights SVD CANNOT:
   1. Which input features are important
   2. Expert specialization patterns
   3. Cross-layer feature consistency
   4. Feature importance validation (ablation)
""")
    
    if output_path:
        analysis_file = os.path.join(output_path, 'comprehensive_analysis.json')
...
def compute_current_cosine_similarities(model: nn.Module, selected_layers: List[int]) -> Dict:
    ...
    for layer_idx in selected_layers:
        ...
        if not hasattr(layer.mlp.experts[0].gate_proj, 'compute_current_cosine_sims'):
            continue
...
def print_cosine_similarity_comparison(
    ...
    if drift < 0.01:
        print(f"  EExcellent!!!: Fine-tuning preserved interpretability almost perfectly")
    elif drift < 0.05:
...
def count_compression_storage(model, selected_layers: List[int]) -> Dict:
    ...
    for layer_idx in selected_layers:
        layer = model.model.layers[layer_idx]
        if hasattr(layer.mlp, 'experts') and hasattr(layer.mlp, 'storage_elements'):
            moe_compressed += layer.mlp.storage_elements()
            moe_original += layer.mlp.original_elements()
...
def main():
    parser = argparse.ArgumentParser(description='Relaxed CUR for MoE Compression (GPU-Optimized)')
    ...
    parser.add_argument('--run_ablations', action='store_true',
                        help='Run additional ablation studies (requires --run_analysis)')
    ...
    print("GPU-OPTIMIZED VERSION with CACHED TEACHER DISTILLATION")
    print(f"\nModel: {args.model}")
    ...
    if not args.skip_finetune:
        if args.reuse_cached_logits:
            ...
            print("\nCACHING TEACHER LOGITS (for memory-efficient fine-tuning)")
            ...
            if args.logits_cache_dir:
                ...
                temp_cache_dir = tempfile.mkdtemp(prefix='relaxed_cur_logits_')
    ...
    print("\nSTAGE 1: Relaxed CUR Compression (GPU-Optimized)")
    ...
    if not args.skip_finetune:
        print("\nSTAGE 2: Knowledge Distillation Fine-tuning (Using Cached Logits)")
    ...
    if args.run_analysis:
        ...
        analysis_results = run_comprehensive_analysis(
            ...
            lr=args.lr,
            lambda_reg=args.lambda_reg,
            tokenizer=tokenizer if args.run_ablations else None,
